...
static constexpr uint16_t const_get_text(std::string_view text)
{
	// index 0 is uninitialized.
	uint16_t index = 1;
#define TL_START(lang, ...) static_assert(std::string_view(#lang) == "English");
#define TL(key, value) \
	if(std::string_view(key) == text) return index; index++;
...
#define _T(x) \
	translate_gettext(([] { static_assert(const_get_text(x) != 0, "translation not found"); }, x))
#else
...
#define TL_START(lang, ...) static_assert(std::string_view(#lang) == "English");
#define TL(key, value) \
	if(hash_fnv1a_const2(key) == hash) return index; index++;
#include "../translations/tl_begin_macro.txt"
#include "../translations/english_ref.inl"
#include "../translations/tl_end_macro.txt"
	return 0;
...
#define _T(x) translate_hash(([]{static_assert(const_get_hash(hash_fnv1a_const2(x)) != 0);},x), hash_fnv1a_const2(x))
